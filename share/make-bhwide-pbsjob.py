...
#Open source template files with simulation configuration
	...
	for line in source_file:
		...
		line = re.sub("TEMPLATE_BEEM_ENERGY", str(2*ENERGY/1e3), line)
...
def do_mc(energy,  event_number,  job_number,  run_number):
	random_seed=int(energy*1000)/10+job_number
	name="bhwide_"+str(energy)+"_"+str(job_number)
	template_dir="/bes3fs/groups/tauqcd/tauqcdgroup/nikolaev/mc/bhwide-template"
	work_dir = name
